...
# bash docker_run_asterisk_pjsip.sh 2 20.10.0

if [ -z "$1" ]; then
  instance_name=asterisk_docker_pjsip-0
  # echo "No instance number. Use 0-9. Example docker_run_asterisk_pjsip.sh 2"
else
  instance_name=asterisk_docker_pjsip-$1
fi
...
${docker_path} run \
  ...
  -v /tmp/:/tmp/ \
  --name "$instance_name" \
  anydict/asterisk20ubuntu22pjsip:"$version"
